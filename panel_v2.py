...
class SerialLink(threading.Thread):
    ...
    def openConnection(self):
        ...
        while port == None:
            ...
            ctr += 1
            if (ctr > 4):
                print("Couldn't find arduino in reasonable time")
                return False
            time.sleep(5)
        ...
        while not closed:
            ...
            try:
                ...
                ctr += 1
                if (ctr > 4):
                    print("Couldn't find open serial port in reasonable time")
                    return False
                time.sleep(5)
        write = False
        ctr = 0
        while not write: # tries to write to the arduino so that the arduino knows to start sending data
            ...
            try:
                ...
                ctr += 1
                if (ctr > 4):
                    print("Couldn't find write to the serial port in reasonable time")
                    return False
        return True
...
def run():
    ''' If "Run" is enabled in panel execute this'''
    global p
